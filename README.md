...
기본적으로 get요청의 응답값은 캐싱이 가능하다. 캐시가 만료됐을시 서버에 요청해서 새롭게 캐싱.
...
서버와 클라이언트간 통신시 클라이언트는  cors헤더를 붙여서 서버로 전송하게 되고 서버측은 responseEntity로 자동으로 해당 헤더값을 형성해서 다시 클라이언트로 전달하게 된다.
